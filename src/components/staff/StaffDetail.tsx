...
export default function StaffDetail({ staffId, isOpen, onClose }: StaffDetailProps) {
  ...
  return (
    <>
      ...
      {/* Modal */}
      <div className="fixed inset-0 z-50 flex items-start justify-center pt-16 p-4 overflow-y-auto">
        <div
        ...
        >
          ...
          {/* Header */}
          <div className="flex items-start justify-between p-6 border-b border-[#E2E8F0] shrink-0">
            <div className="flex items-center gap-4">
              <div className="w-14 h-14 rounded-full bg-[#0F6FDE] text-white flex items-center justify-center text-xl font-bold shrink-0">
                {staff.name.charAt(0)}
              </div>
              <div>
                <h2 className="text-lg font-bold text-[#1E293B]">{staff.name}</h2>
            ...
            {activeTab === 'basic' && (
              <div className="space-y-0">
                {basicInfoRows.map((row) => (
                  <div
                    key={row.label}
                    className="grid grid-cols-[140px_1fr] py-3 border-b border-[#E2E8F0] last:border-b-0"
                  >
